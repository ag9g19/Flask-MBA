...
def iup_contour(delta, coords):
    ...
    if start != 0:
        ...
        out.extend(
            iup_segment(coords[i1:i2], coords[ri1], delta[ri1], coords[ri2], delta[ri2])
        )
    ...
    for end in it:
        if end - start > 1:
            ...
            out.extend(
                iup_segment(
                    coords[i1:i2], coords[ri1], delta[ri1], coords[ri2], delta[ri2]
                )
            )
...
def iup_contour_optimize(delta, coords, tolerance=0.0):
    n = len(delta)
    ...
    if forced:
        ...
        chain, costs = _iup_contour_optimize_dp(
            delta + delta, coords + coords, forced, tolerance, n
        )
...
def iup_delta_optimize(delta, coords, ends, tolerance=0.0):
    assert sorted(ends) == ends and len(coords) == (ends[-1] + 1 if ends else 0) + 4
    n = len(coords)
    ends = ends + [n - 4, n - 3, n - 2, n - 1]
    out = []
    ...
    for end in ends:
        contour = iup_contour_optimize(
            delta[start : end + 1], coords[start : end + 1], tolerance
        )
